...
`;
    ...
    function Accordion(props) {
      ...
      // On open, ensure the provider that holds the current model is expanded, so
      // reopening lands on the model you are actually using rather than whatever
      // provider you happened to expand last time. Other manually-open providers
      // are preserved.
      React.useEffect(() => {
        if (!open) return;
        const g = groups.find((gg) => gg.id === state.current?.provider);
        if (g && !(g.id in expanded)) setExpanded((old) => ({ ...old, [g.id]: true }));
      }, [open, state.current?.provider]);

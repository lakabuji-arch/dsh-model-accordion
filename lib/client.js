...
`;
    ...
    function Accordion(props) {
      ...
      // On open, ensure the provider holding the current model is expanded, so
      // reopening lands on the model you are using. Deps are [open] ONLY (not the
      // provider id): re-running on every selection during a search-assisted
      // multi-model test round would accumulate every tested provider into
      // `expanded`, leaving them all open after the filter is cleared.
      React.useEffect(() => {
        if (!open) return;
        const g = groups.find((gg) => gg.id === state.current?.provider);
        if (g && !(g.id in expanded)) setExpanded((old) => ({ ...old, [g.id]: true }));
      }, [open]);
      ...
      // When the filter goes from active (non-empty) to empty — via ✕, Escape, or
      // manual deletion — collapse every provider except the current model's. This
      // prevents a finished multi-model test round (many providers auto-expanded by
      // search) from leaving every directory open once the filter is cleared.
      const prevQuery = React.useRef("");
      React.useEffect(() => {
        const prev = prevQuery.current;
        prevQuery.current = q;
        if (q === "" && prev !== "" && state.current) {
          const g = groups.find((gg) => gg.id === state.current?.provider);
          setExpanded(g ? { [g.id]: true } : {});
        }
      }, [q]);
